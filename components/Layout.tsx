...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans text-clinic-blue bg-clinic-bg">
      ...
      <header className="fixed top-5 left-0 right-0 mx-auto w-[95%] md:w-[90%] max-w-[1400px] h-[100px] md:h-[120px] bg-white/15 backdrop-blur-md border border-white/20 rounded-[35px] flex justify-between items-center px-4 md:px-[30px] z-50 shadow-lg transition-all hover:bg-white/20 hover:shadow-2xl">
        ...
        <Link to="/" className="z-50 flex-shrink-0">
          <img 
            ...
            className="h-[60px] w-[60px] md:h-[90px] md:w-[90px] object-contain transition-transform hover:scale-105"
          />
        </Link>
           ...
           <div className="flex gap-3 md:gap-8 animate-scroll hover:pause w-max">
            {displayStories.map((story: Story, index: number) => (
              ...
              >
                ...
                <span className="text-[11px] md:text-sm font-semibold text-clinic-blue mt-1 truncate max-w-[70px] drop-shadow-sm">{story.title}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div 
          className={`flex flex-col justify-between w-[30px] h-[22px] cursor-pointer z-50 flex-shrink-0 ${isMenuOpen ? 'active' : ''}`}
          onClick={() => setIsMenuOpen(!isMenuOpen)}
        >
          <span className={`w-full h-[3px] bg-gradient-to-r from-clinic-blue to-clinic-lime rounded-full transition-all duration-300 ${isMenuOpen ? 'translate-y-[9px] rotate-45' : ''}`}></span>
          <span className={`w-full h-[3px] bg-gradient-to-r from-clinic-blue to-clinic-lime rounded-full transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`}></span>
          <span className={`w-full h-[3px] bg-gradient-to-r from-clinic-blue to-clinic-lime rounded-full transition-all duration-300 ${isMenuOpen ? '-translate-y-[9px] -rotate-45' : ''}`}></span>
        </div>
      </header>
      ...
      {viewingStory && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex flex-col justify-center items-center animate-fade-in-up">
          <button 
            className="absolute top-6 right-6 text-white text-4xl hover:text-clinic-lime transition-colors z-[101]"
            ...
            {navigation.map((item: any, index: number) => (
              <li key={index} style={{ transitionDelay: `${index * 100}ms` }} className={`transform transition-all duration-500 ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                {item.path.startsWith('http') ? (
                  <a href={item.path} target="_blank" rel="noreferrer" className="font-sans text-3xl font-semibold text-clinic-blue hover:text-clinic-lime transition-colors">
                    {item.label}
                  </a>
                ) : (
                  <Link to={item.path} className="font-sans text-3xl font-semibold text-clinic-blue hover:text-clinic-lime transition-colors">
                    {item.label}
                  </Link>
                )}
              </li>
      ...
      {/* Main Content */}
      <main className="flex-grow pt-[130px] md:pt-[160px] relative">
        ...
        <div className="max-w-[1400px] mx-auto">
          <div className="text-center mb-[50px]">
             <div className="inline-block mb-8">
               <img src="https://clinicasantamariadosolivais.pt/wp-content/uploads/2025/06/Recurso-6.png" alt="Logo Footer" className="max-w-[150px] md:max-w-[200px] mx-auto" />
             </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 mb-10 text-center md:text-left">
            <div className="flex flex-col gap-8 md:items-start items-center">
              <div>
                <h4 className="text-xl font-medium text-clinic-purple mb-2">Redes sociais</h4>
              ...
              <div>
                <h4 className="text-xl font-medium mb-2">Localização</h4>
                <a href={global.mapsLink || "#"} target="_blank" rel="noreferrer" className="text-lg text-clinic-purple font-light hover:text-white transition-colors">
                  {global.address}
                </a>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-8 md:items-end items-center text-center md:text-right">
              ...
              <div>
                <h4 className="text-xl font-medium mb-2">Whatsapp</h4>
                <a href={global.socials?.whatsapp || "#"} target="_blank" rel="noreferrer" className="text-lg text-clinic-purple font-light hover:text-white transition-colors">{global.mobile}</a>
                <p className="text-sm text-gray-400 mt-1">(Preço de uma chamada de Rede Móvel Nacional)</p>
              </div>
            </div>
          </div>
