...
// UPDATED: Deeper U-Waves (65px amplitude) and wider frequency (0.18)
const CarnivalBunting: React.FC = React.memo(() => {
  // Generate static data once. 
  const flagsData = useMemo(() => {
    return Array.from({ length: 42 }).map((_, i) => { // Increased to 42 for fuller coverage
      // Natural wind sway parameters
      const duration = 3 + Math.random() * 2; // Slower, more elegant sway
      const delay = Math.random() * -5; 
      
      // "Extreme Deep U Waves" (Guirnalda Muy Pronunciada)
      // 0.18 frequency makes the U wider / 65 amplitude makes the U very deep
      const waveOffset = Math.abs(Math.sin(i * 0.18)) * 65; 
      // Increased rotation to match the steepness of the curve
      const rotation = Math.cos(i * 0.18) * 35;
  ...
  return (
    <div className="absolute top-[88%] left-0 w-full flex justify-between items-start overflow-hidden pointer-events-none z-[40] px-0 md:px-2">
       {flagsData.map((flag) => (
           <div 
             key={flag.i}
           ...
           >
             ...
             <div 
                className="absolute -top-[18px] left-1/2 -translate-x-1/2 w-[160%] h-[4px] origin-center shadow-md" 
                style={{ 
                  transform: `rotate(${flag.rotation}deg)`,
                  // Create a twisted rope pattern using gradients (Realist Rope)
                  background: 'repeating-linear-gradient(45deg, #bfbfbf, #bfbfbf 2px, #e8e8e8 3px, #e8e8e8 5px)',
                  borderRadius: '2px'
                }}
             ></div>
             ...
             <div className="relative group perspective-500">
               ...
               {/* Fabric Body */}
               <div 
                  className="w-[36px] h-[52px] md:w-[44px] md:h-[68px] shadow-xl transition-transform"
                  style={{
                    ...
                    backgroundImage: `
                      linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0) 15%), 
                      linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.25) 50%, rgba(0,0,0,0.15) 100%),
               ...
               {/* The Emblem: Gold Button/Pin Look (Hyper-realistic) */}
               <div 
                  className="absolute top-[10px] left-1/2 -translate-x-1/2 w-[12px] h-[12px] md:w-[15px] md:h-[15px] rounded-full z-20 shadow-md"
                  style={{
                    background: 'radial-gradient(circle at 30% 30%, #fffebb, #ffd700, #b8860b)', // Gold metallic gradient
               ...
               <div className="absolute top-0 left-0 w-full h-[5px] bg-black/15 mix-blend-multiply rounded-t-sm"></div>
             </div>
