...
const Home: React.FC = () => {
    ...
    const syncPlayback = () => {
      videoRefs.current.forEach((v, idx) => {
        ...
        if (isCenter && isStoriesVisible) {
          // Priority for the foreground video
          v.muted = false;
          v.preload = "auto";
          const playPromise = v.play();
          if (playPromise !== undefined) {
            playPromise.catch(() => {
              v.muted = true; // Fallback for browsers blocking sound until interaction
              v.play().catch(() => {});
            });
          }
        } else {
          // Pause and silence background videos to optimize performance and prevent audio overlap
          v.muted = true;
          v.pause();
          if (!isCenter) {
            v.currentTime = 0; // Reset hidden videos
            v.preload = "metadata";
          }
        }
      });
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Keep interaction state for a while to prevent immediate auto-rotate after manual swipe
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      {/* Stories Section - INTENSIFIED LOCALIZED BACKGROUND GLOW LIGHTS */}
      <section 
      ...
      >
        {/* Potent Localized Background Glows behind the center story */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[500px] aspect-square pointer-events-none opacity-55 z-0">
           <div className="absolute inset-0 rounded-full blur-[80px] animate-glow-cycle"></div>
           <div className="absolute inset-10 rounded-full blur-[60px] animate-glow-cycle opacity-90" style={{ animationDelay: '-4s' }}></div>
        </div>
      ...
      {/* Experience Section - ULTIMATE POTENT LOCALIZED GLOW LIGHTS */}
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center relative z-10">
          <div className="flex flex-col items-center lg:items-end order-2 lg:order-1 gap-8">
             <div className="relative group/video w-full max-w-[245px] md:max-w-[340px] aspect-[9/16] rounded-[2.8rem] md:rounded-[4.5rem] p-[2px] overflow-visible">
                ...
                {/* INTENSIFIED LOCALIZED GLOW LIGHTS BEHIND VIDEO */}
                <div className="absolute -inset-8 md:-inset-16 rounded-full blur-[60px] animate-glow-cycle opacity-85"></div>
                <div className="absolute -inset-4 md:-inset-8 bg-gradient-to-tr from-clinic-blue/90 via-clinic-lime/80 to-clinic-purple/90 rounded-[3.2rem] md:rounded-[5.5rem] opacity-100 blur-[35px] animate-pulse"></div>
                
                {/* Localized Potent Floating Soft Orbs */}
                <div className="absolute -top-10 -left-10 w-40 h-40 bg-clinic-lime/45 rounded-full blur-[40px] animate-float" style={{ animationDuration: '10s' }}></div>
                <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-clinic-purple/45 rounded-full blur-[40px] animate-float" style={{ animationDuration: '8s', animationDelay: '2s' }}></div>
             ...
             <Link to="/campanhas" className="bg-clinic-purple text-white px-10 py-4 rounded-full text-xl font-bold shadow-xl hover:bg-clinic-blue transition-all transform hover:scale-105 active:scale-95">
