  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <section className="text-center px-4 mb-20">
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-semibold text-clinic-blue mb-4 leading-tight">
          {/* Dynamically render title if it includes "Clínica", otherwise just show text */}
          {heroTitle.includes('Clínica') ? (
            <>
              <span className="font-body text-clinic-purple text-2xl md:text-3xl font-medium mr-2">Clínica</span>
        ...
        <p className="text-lg md:text-xl text-gray-800 max-w-3xl mx-auto mb-8 font-light">
          {heroSubtitle}
        </p>
        <Link 
          to="/contactos" 
          className="inline-block bg-clinic-lime text-clinic-blue font-semibold text-lg px-8 py-3 rounded-full hover:bg-clinic-purple hover:text-white transition-all transform hover:-translate-y-1 shadow-lg"
        >
      ...
      <section className="px-4 py-10 md:py-20 bg-transparent relative overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
          ...
          <div className="flex flex-col items-center lg:items-end order-2 lg:order-1 gap-6">
             <div className="relative w-[280px] sm:w-[320px] h-auto rounded-[2.5rem] overflow-hidden border-[6px] border-white shadow-2xl bg-black group transform transition-transform hover:scale-[1.02] duration-500">
                ...
                <video 
                   ...
                   src="https://res.cloudinary.com/dgvavcxlz/video/upload/w_600,f_auto,q_auto,vc_auto/v1768688465/IMPLANTES_LISTO_bdwqnq.mp4"
                   // Generated Poster (first frame) for immediate visual feedback
          ...
          {/* Right: Text Content (Original) */}
          <div className="text-center lg:text-left order-1 lg:order-2 flex flex-col items-center lg:items-start">
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold text-clinic-blue leading-tight flex flex-col items-center lg:items-start gap-4">
              <div className="flex flex-wrap justify-center lg:justify-start items-baseline gap-2 md:gap-3">
                ...
                <span className="font-serif italic text-clinic-lime text-5xl md:text-8xl">10</span>
              ...
              <div className="flex flex-wrap justify-center lg:justify-start items-center gap-2 md:gap-3">
                <span>o seu</span>
                <div className="w-[80px] h-[60px] sm:w-[100px] sm:h-[70px] md:w-[150px] md:h-[100px] rounded-2xl overflow-hidden border-4 border-white shadow-lg mx-1 md:mx-2">
                  <img src="https://clinicasantamariadosolivais.pt/wp-content/uploads/2025/05/20250528_1139_Pose-Divertida_remix_01jwb7hqzte4ma70g3g1ezq96d.png" alt="Sorriso" className="w-full h-full object-cover animate-float" />
                </div>
              ...
              <div className="flex flex-wrap justify-center lg:justify-start items-center gap-2 md:gap-3">
                <span>de sonho</span>
                <div className="w-[80px] h-[60px] sm:w-[100px] sm:h-[70px] md:w-[150px] md:h-[100px] rounded-2xl overflow-hidden border-4 border-white shadow-lg mx-1 md:mx-2">
                  <img src="https://clinicasantamariadosolivais.pt/wp-content/uploads/2025/05/Sem-titulo-68.png" alt="Sonho" className="w-full h-full object-cover animate-float" style={{ animationDelay: '1.5s' }} />
                </div>
              </div>
              <div className="flex flex-wrap justify-center lg:justify-start items-center gap-2 md:gap-3">
                <span>é o</span>
                <span className="font-serif italic">nosso cuidado</span>
                <div className="w-[40px] h-[40px] md:w-[50px] md:h-[50px] rounded-full bg-clinic-blue text-white flex items-center justify-center shadow-lg animate-pulse-slow">♥</div>
              </div>
            </h2>
          </div>
      ...
      <section className="max-w-[1000px] mx-auto px-4 py-20 relative">
        ...
        <h2 className="text-3xl md:text-6xl font-serif italic text-center mb-16 border-2 border-clinic-lime inline-block px-10 py-4 rounded-full mx-auto block w-fit bg-white/50 backdrop-blur-sm shadow-sm">
          Nossos Serviços
        </h2>
        ...
        <div className="grid gap-5">
          {[
            ...
            <details key={index} className="group bg-white rounded-2xl border border-clinic-blue/10 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:border-clinic-lime/50 open:shadow-xl open:border-clinic-lime open:-translate-y-1">
              <summary className="flex justify-between items-center cursor-pointer p-6 list-none transition-colors relative">
                {/* Accent Line on Left */}
                ...
                <div className="flex items-center gap-4 md:gap-6 pl-2">
                   {/* Numbering */}
                  <span className="text-3xl md:text-4xl font-serif italic text-clinic-lime/30 font-bold group-hover:text-clinic-lime group-open:text-clinic-lime transition-colors">
                    0{index + 1}
                  </span>
                  <span className="text-lg md:text-2xl font-semibold text-clinic-blue group-open:text-clinic-purple transition-colors">
                    {service.title}
                  </span>
                </div>

                <span className="w-10 h-10 rounded-full bg-clinic-bg flex items-center justify-center text-clinic-blue transform transition-all group-open:rotate-180 group-open:bg-clinic-blue group-open:text-white group-hover:scale-110 shadow-sm">
                  <i className="fas fa-chevron-down"></i>
                </span>
              </summary>
              <div className="p-8 pt-2 text-clinic-blue bg-gradient-to-b from-white to-gray-50 border-t border-gray-100">
                <div className="flex flex-col md:flex-row gap-8 items-center">
                    <div className="flex-1">
                        <p className="text-base md:text-lg leading-relaxed text-gray-600 mb-6">{service.text}</p>
                        
                        {/* BOTÓN CAMBIO REDIRECCION */}
                        <Link 
                          to="/marcacoes" 
                          state={{ service: service.title }}
                          className="inline-block bg-clinic-lime text-clinic-blue font-bold text-sm px-6 py-3 rounded-full shadow-md hover:bg-clinic-blue hover:text-white transition-all transform hover:-translate-y-1"
                        >
                          Agendar Avaliação
                        </Link>
                    </div>
                    <img src={service.img} alt={service.title} className="w-full md:w-[250px] rounded-xl shadow-lg object-cover h-[180px] transition-transform duration-700 hover:scale-105" />
                </div>
              </div>
            </details>
          ))}
      ...
      <section className="py-20 text-center">
        <div className="max-w-[1000px] mx-auto px-4">
          <h2 className="text-3xl md:text-5xl font-bold text-clinic-blue mb-8 flex flex-wrap justify-center items-center gap-4">
            <span>Transforme seu</span>
            <div className="w-[80px] h-[60px] md:w-[100px] md:h-[70px] rounded-xl overflow-hidden border-2 border-white shadow-lg">
                <img src="https://clinicasantamariadosolivais.pt/wp-content/uploads/2025/05/Sem-titulo-68.png" alt="Sonho" className="w-full h-full object-cover animate-float" />
            </div>
            <span className="font-serif italic">sorriso</span>
            <span>hoje!</span>
          </h2>
          <Link 
            to="/marcacoes" 
            className="inline-flex items-center gap-2 bg-clinic-purple text-white px-8 py-4 rounded-full text-xl font-semibold shadow-lg hover:bg-clinic-blue transition-all hover:-translate-y-1 transform"
          >
