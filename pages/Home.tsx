...
const Home: React.FC = () => {
  ...
  // Listener global para habilitar audio tras la primera interacción (bypass de políticas de navegador)
  useEffect(() => {
    const enableAudio = () => {
  ...
  // Forzar la carga de todos los vídeos al montar la página
  useEffect(() => {
    const preloadAll = () => {
      videoRefs.current.forEach(v => { if (v) v.load(); });
      if (expVideoRef.current) expVideoRef.current.load();
    };
    const timer = setTimeout(preloadAll, 500);
    return () => clearTimeout(timer);
  }, [stories]);

  // Manejo de temporizador para imágenes
  useEffect(() => {
  ...
  // Intersection Observer para control de audio y reproducción por scroll
  useEffect(() => {
          ...
          if (entry.target === expVideoRef.current) {
            if (entry.isIntersecting) {
              const playVideo = async () => {
                ...
                try {
                  // Intentar con audio si el usuario ya interactuó
                  ...
                  if (expVideoRef.current) {
                    expVideoRef.current.muted = true;
                    setIsExpMuted(true);
  ...
  // Sincronización de reproducción e historias: Audio automático en el centro
  useEffect(() => {
    const syncPlayback = async () => {
      for (let i = 0; i < videoRefs.current.length; i++) {
        ...
        if (isCenter && isStoriesVisible) {
          try {
            v.muted = !hasInteracted; // Activar audio si hubo interacción
            await v.play();
          } catch (e) {
            v.muted = true; 
            v.play().catch(() => {});
  ...
  return (
    <div className="animate-fade-in-up">
      {/* Hero Header - Sin Botón (Movido abajo) */}
      <section className="text-center px-4 mb-8 md:mb-12">
        ...
        <p className="text-base md:text-xl text-gray-800 max-w-3xl mx-auto font-light px-4">
          {heroSubtitle}
        </p>
      </section>

      {/* Stories Section - FIXED LOADING & NO ERROR */}
      <section 
        ref={storiesSectionRef}
        className="relative h-[340px] md:h-[500px] max-w-[1400px] mx-auto overflow-visible mb-8 flex justify-center items-center"
        onTouchStart={handleTouchStart}
        onTouchEnd={handleTouchEnd}
      >
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 md:w-96 md:h-96 pointer-events-none z-0">
           <div className="absolute inset-[-40%] rounded-full blur-[90px] md:blur-[130px] animate-glow-cycle opacity-40"></div>
           <div className="absolute inset-0 rounded-full blur-[60px] md:blur-[90px] animate-glow-cycle opacity-50" style={{ animationDelay: '-6s' }}></div>
        </div>
        ...
        <div className="relative w-full h-full flex justify-center items-center perspective-[1500px]">
          {stories.map((story: any, index: number) => {
            ...
            return (
              <div 
                key={story.id} 
                className={`absolute w-[125px] h-[210px] md:w-[200px] md:h-[330px] rounded-[2.2rem] md:rounded-[3.2rem] border-[4px] md:border-[8px] border-white shadow-2xl overflow-hidden bg-black cursor-pointer transition-all duration-500 ${isCenter ? 'ring-4 ring-clinic-lime/50 shadow-clinic-lime/20' : ''}`}
              ...
              >
                {story.type === 'video' ? (
                  ...
                  <img src={story.thumbnail || story.src} className="w-full h-full object-cover" loading="eager" />
                )}
                {isCenter && (
      ...
      {/* CONTROL POINT: Botón Principal Centrado Debajo de Historias */}
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center relative z-10">
          <div className="flex flex-col items-center lg:items-end order-2 lg:order-1 gap-8">
             <div className="relative w-full max-w-[245px] md:max-w-[340px] aspect-[9/16] rounded-[2.8rem] md:rounded-[4.5rem] flex items-center justify-center overflow-visible">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[80%] rounded-full blur-[100px] md:blur-[150px] animate-glow-cycle opacity-50 -z-10"></div>
                <div className="relative w-full h-full rounded-[2.6rem] md:rounded-[4.2rem] overflow-hidden border-[6px] md:border-[12px] border-white shadow-2xl bg-black">
                  ...
                  <button 
                    ...
                    className="absolute bottom-6 right-6 z-20 w-12 h-12 bg-white/25 backdrop-blur-lg rounded-full border border-white/40 flex items-center justify-center text-white hover:bg-white hover:text-clinic-blue transition-all shadow-xl"
                  >
                    <i className={`fas ${isExpMuted ? 'fa-volume-xmark' : 'fa-volume-high'}`}></i>
                  </button>
                </div>
             </div>
             <Link to="/campanhas" className="bg-clinic-purple text-white px-8 py-4 rounded-full text-lg font-bold shadow-xl hover:bg-clinic-blue transition-all transform hover:scale-105 active:scale-95">
          ...
          {["IMPLANTOLOGIA", "ORTODONTIA, ALINHADORES INVISÍVEIS", "LIMPEZA E DESTARTARIZAÇÃO", "CONSULTA DE ROTINA", "ODONTOPEDIATRIA", "BRANQUEAMENTO"].map((title, index) => (
            <details key={index} className="group bg-white rounded-3xl border border-clinic-blue/5 shadow-sm overflow-hidden transition-all duration-500 hover:shadow-xl open:border-clinic-lime">
              ...
              <div className="px-8 pb-8 pt-2">
                <p className="text-lg leading-relaxed text-gray-600 mb-8">Tratamento especializado con los más elevados estándares de calidad clínica.</p>
      ...
      <section className="py-20 text-center">
        <div className="max-w-[1100px] mx-auto px-4 flex flex-col items-center">
          ...
          <Link 
            to="/marcacoes" 
            className="inline-block bg-clinic-blue text-white px-6 py-4 rounded-full text-base font-bold shadow-2xl hover:bg-clinic-purple transition-all transform hover:-translate-y-2 text-center w-full max-w-[320px] sm:w-auto sm:max-w-none sm:px-12 sm:py-5 sm:text-xl active:scale-95"
          >
