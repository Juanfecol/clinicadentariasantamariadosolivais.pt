...
const Home: React.FC = () => {
  ...
  // Intersection Observer for both Sections to manage audio/playback based on scroll
  useEffect(() => {
        ...
        entries.forEach(entry => {
          ...
          if (entry.target === expVideoRef.current) {
            if (entry.isIntersecting) {
              ...
              // Auto-mute when scrolling away to be safe
              setIsExpMuted(true);
            }
          }
        });
  ...
  // STRICT VIDEO ENGINE: Ensures ONLY the center video plays and has sound when visible
  useEffect(() => {
    const syncPlayback = () => {
    ...
    <div className="animate-fade-in-up">
      ...
      <section className="text-center px-4 mb-16 md:mb-24">
        ...
        <Link 
          ...
          className="inline-block bg-clinic-lime text-clinic-blue font-bold text-lg px-8 sm:px-10 py-4 rounded-full hover:bg-clinic-purple hover:text-white transition-all transform hover:-translate-y-1 shadow-xl"
        >
          Marque a Sua Consulta Hoje
        </Link>
      </section>

      {/* Stories Section - CONCENTRATED TIGHT GLOW */}
      <section 
      ...
      >
        {/* Aura Ajustada - Menos intrusiva, sin formas rectangulares */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[130px] md:w-[210px] aspect-[9/16] pointer-events-none opacity-50 z-0">
           <div className="absolute -inset-2 md:-inset-4 rounded-full blur-[35px] md:blur-[45px] animate-glow-cycle"></div>
           <div className="absolute inset-4 rounded-full blur-[25px] md:blur-[35px] animate-glow-cycle opacity-60" style={{ animationDelay: '-6s' }}></div>
        </div>
        ...
        <div className="relative w-full h-full flex justify-center items-center perspective-[1500px]">
          {stories.map((story: any, index: number) => {
            ...
            return (
              <div 
                key={story.id} 
                className={`absolute w-[125px] h-[210px] md:w-[200px] md:h-[330px] rounded-[2.2rem] md:rounded-[3.2rem] border-[4px] md:border-[8px] border-white shadow-2xl overflow-hidden bg-black cursor-pointer select-none transition-all duration-500 ${isCenter ? 'ring-4 ring-clinic-lime/50 shadow-clinic-lime/30' : ''}`}
              ...
              >
                {story.type === 'video' ? (
                  <div className="w-full h-full relative">
                    ...
                    />
                    {isCenter && (
                      <div className="absolute bottom-0 left-0 w-full h-1.5 bg-black/30 z-20">
                        <div 
                          className="h-full bg-white transition-all duration-100 ease-linear shadow-[0_0_12px_rgba(255,255,255,0.9)]" 
                ...
                <div className={`absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/95 to-transparent transition-opacity duration-500 ${isCenter ? 'opacity-100' : 'opacity-0'}`}>
                  <p className="text-white text-[10px] md:text-xs font-bold uppercase tracking-widest text-center drop-shadow-md">{story.title}</p>
                </div>
              </div>
            );
          })}
      ...
      {/* Experience Section - REFINED BORDER HALO */}
      <section className="px-4 py-16 md:py-32 relative overflow-hidden" aria-labelledby="experience-heading">
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center relative z-10">
          <div className="flex flex-col items-center lg:items-end order-2 lg:order-1 gap-8">
             <div className="relative group/video w-full max-w-[245px] md:max-w-[340px] aspect-[9/16] rounded-[2.8rem] md:rounded-[4.5rem] p-[2px] overflow-visible">
                
                {/* Aura Ultra-Ajustada - Apenas sobresale para un toque de color premium */}
                <div className="absolute -inset-3 md:-inset-5 rounded-full blur-[30px] md:blur-[40px] animate-glow-cycle opacity-40"></div>
                <div className="absolute inset-2 md:inset-4 bg-gradient-to-tr from-clinic-blue via-clinic-lime to-clinic-purple rounded-full opacity-20 blur-[20px] animate-pulse"></div>
                
                {/* Accent Orbs - Suaves */}
                <div className="absolute -top-4 -left-4 w-24 h-24 bg-clinic-lime/10 rounded-full blur-[40px] animate-float" style={{ animationDuration: '6s' }}></div>
                <div className="absolute -bottom-4 -right-4 w-24 h-24 bg-clinic-purple/10 rounded-full blur-[40px] animate-float" style={{ animationDuration: '5s', animationDelay: '1s' }}></div>
                ...
                <div className="relative w-full h-full rounded-[2.6rem] md:rounded-[4.2rem] overflow-hidden border-[6px] md:border-[12px] border-white shadow-2xl bg-black">
                  ...
                  <button 
                    ...
                    className="absolute bottom-6 right-6 z-20 w-12 h-12 md:w-16 md:h-16 bg-white/25 backdrop-blur-lg rounded-full border border-white/40 flex items-center justify-center text-white hover:bg-white hover:text-clinic-blue transition-all shadow-xl active:scale-90"
                    aria-label={isExpMuted ? "Ativar som" : "Desativar som"}
                  >
                    <i className={`fas ${isExpMuted ? 'fa-volume-xmark' : 'fa-volume-high'} text-lg md:text-xl`}></i>
                  </button>
                </div>
             </div>
             <Link to="/campanhas" className="bg-clinic-purple text-white px-8 sm:px-10 py-4 rounded-full text-lg sm:text-xl font-bold shadow-xl hover:bg-clinic-blue transition-all transform hover:scale-105 active:scale-95 text-center">
      ...
      <section className="py-20 text-center">
        <div className="max-w-[1100px] mx-auto px-4">
          ...
          <Link to="/marcacoes" className="inline-block bg-clinic-blue text-white px-8 sm:px-12 py-4 sm:py-5 rounded-full text-base sm:text-xl font-bold shadow-2xl hover:bg-clinic-purple transition-all transform hover:-translate-y-2 text-center whitespace-normal">
