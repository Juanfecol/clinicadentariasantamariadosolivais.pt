...
const ThankYou: React.FC = () => {
  ...
  // SEM OPTIMIZATION: Fire conversion event on Page Load to guarantee tracking
  // This is the most reliable way to train Google/Meta algorithms.
  useEffect(() => {
    // Google Ads Conversion (Redundant but safe)
    if (typeof (window as any).gtag === 'function') {
      (window as any).gtag('event', 'conversion', {
        'send_to': 'AW-1135006626/form_submission',
        'value': 10.0,
        'currency': 'EUR',
        'transaction_id': `lead_${Date.now()}` // Unique ID to prevent deduplication errors
      });
    }

    // Meta Pixel Conversion (Standard Lead event)
    if (typeof (window as any).fbq === 'function') {
      (window as any).fbq('track', 'Lead', {
        value: 10.0,
        currency: 'EUR',
        content_name: displayData.servico || 'General Inquiry'
      });
    }
  }, [displayData.servico]);
